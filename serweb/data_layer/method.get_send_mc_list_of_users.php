<?
/*
 * $Id: method.get_send_mc_list_of_users.php,v 1.3 2005/11/01 17:58:39 kozlik Exp $
 */
	...
	function get_send_MC_list_of_users(&$errors){
		...
		if ($config->users_indexed_by=='uuid'){
			$q="select s.username, s.domain, s.uuid, s.email_address, p.value ".
				"from ".$config->data_sql->table_subscriber." s left outer join ".$config->data_sql->table_user_preferences." p ".$flags['deleted']['from'].
		...
		$q .= " where ".$flags['deleted']['where']." ".$this->get_sql_bool(true);
