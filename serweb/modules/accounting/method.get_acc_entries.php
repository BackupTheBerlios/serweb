 ...
 *      Please note that combining multiple types of calls (incoming, outgoing 
 *      and missed) in the result could lead to performance problems
 *
 ...
 *      and missed) in the result could lead to performance problems
 *
 *    get_unpaired_bye      (bool) default: false
 *      Include unpaired BYE records in the result. Please note that including 
 *      unpaired BYE records could lead to performance problems.
 *
 ...
 */       
    ...
    function get_acc_entries($user, $opt, &$errors){
        ...
        $opt_unpaired_bye    = (isset($opt['get_unpaired_bye']))   ? (bool)$opt['get_unpaired_bye'] : false;
        ...
        $this->acc_prepare_SQL($opt_unpaired_bye);
        ...
        if ($opt_filter_outgoing) 
            $q=array_merge($q, $this->acc_get_SQL_select_outgoing_count($user, $opt_unpaired_bye));

        if ($opt_filter_incoming) 
            $q=array_merge($q, $this->acc_get_SQL_select_incoming_count($user, $opt_unpaired_bye));
        ...
        $q = array();
        if ($opt_filter_outgoing) 
            $q[] = $this->acc_get_SQL_select_outgoing($user, $opt_unpaired_bye);
            
        if ($opt_filter_incoming) 
            $q[] = $this->acc_get_SQL_select_incoming($user, $opt_unpaired_bye);
...
//              $o['length']=0;
    ...
    function acc_prepare_SQL($unpaired_bye){
        global $config;
        ...
        $t_acc = &$config->data_sql->acc->table_name;
            ...
            */
            ...
            if ($unpaired_bye)
                $this->acc_sql['select_out'] .= "coalesce(t1.response_timestamp, t2.request_timestamp) as ttime ";
            else
                /* When result will not contain unpaired BYE records, 
                   t1.response_timestamp never is NULL. So coalesce function 
                   could be safely ommited and the sorting by ttime is much faster*/
                $this->acc_sql['select_out'] .= "t1.response_timestamp as ttime ";
            ...
            if ($unpaired_bye)
                $this->acc_sql['select_in'] .= "coalesce(t1.response_timestamp, t2.request_timestamp) as ttime ";
            else
                /* When result will not contain unpaired BYE records, 
                   t1.response_timestamp never is NULL. So coalesce function 
                   could be safely ommited and the sorting by ttime is much faster*/
                $this->acc_sql['select_in'] .= "t1.response_timestamp as ttime ";
    ...
    function acc_get_SQL_select_outgoing($user, $unpaired_bye){
        global $config;
        ...
        $q = "(".$this->acc_sql['select_out'].", 'outgoing' as call_type ".
                 $this->acc_sql['from_1'].
                 $this->acc_sql['where_1']." and t1.from_uid = ".$this->sql_format($user->get_uid(), 's').
                        " and (t1.flags & ".$f_acc['DB_CALLER_DELETED']." = 0) ".
             ")";

        if ($unpaired_bye){
            $q .= " union (".   // get unpaired BYE records
                     $this->acc_sql['select_out'].", 'outgoing' as call_type ".
                     $this->acc_sql['from_2'].
                     $this->acc_sql['where_2']." and t2.to_uid = ".$this->sql_format($user->get_uid(), 's').
    ...
    function acc_get_SQL_select_outgoing_count($user, $unpaired_bye){
        global $config;
    ...
    function acc_get_SQL_select_incoming($user, $unpaired_bye){
        global $config;
        ...
        $q = "(".$this->acc_sql['select_in'].", 'incoming' as call_type ".
                 $this->acc_sql['from_1'].
                 $this->acc_sql['where_1']." and t1.to_uid = ".$this->sql_format($user->get_uid(), 's').
        ...
        if ($unpaired_bye){
            $q .= " union (".   // get unpaired BYE records
                     ...
                     $this->acc_sql['from_2'].
                     $this->acc_sql['where_2']." and t2.from_uid = ".$this->sql_format($user->get_uid(), 's').
    ...
    function acc_get_SQL_select_incoming_count($user, $unpaired_bye){
        global $config;
    ...
    function acc_get_SQL_select_missed($user){
        ...
        $q="(".$this->acc_sql['select_missed'].", 'missed' as call_type ".
                "FROM ".$t_mc." t1 ".
                "WHERE t1.to_uid = ".$this->sql_format($user->get_uid(), 's')." and 
