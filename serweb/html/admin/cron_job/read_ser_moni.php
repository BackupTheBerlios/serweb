<?
/*
 * $Id: read_ser_moni.php,v 1.2 2003/03/17 18:18:35 kozlik Exp $
 */
...
do{
	...
	if (ereg("[^0-9]*([0-9]+)[^0-9]*([0-9]+)[^0-9]*([0-9]+)[^0-9]*([0-9]+)", $out_l, $regs)){
		...
		$sm=new Ser_moni("ts_current");
		$sm->update($regs[1]);
		
		$sm=new Ser_moni("ts_waiting");
		$sm->update($regs[2]);

		$sm=new Ser_moni("ts_total");
		$sm->update($regs[3]);

		$sm=new Ser_moni("ts_total_local");
		$sm->update($regs[4]);
...
//		$q_part2.=", ".$regs[1];
	...
	if (ereg("[^0-9]*6xx: ([0-9]+)[^0-9]*5xx: ([0-9]+)[^0-9]*4xx: ([0-9]+)[^0-9]*3xx: ([0-9]+)[^0-9]*2xx: ([0-9]+)", $out_l, $regs)){
		...
		$sm=new Ser_moni("ts_6xx");
		$sm->update($regs[1]);

		$sm=new Ser_moni("ts_5xx");
		$sm->update($regs[2]);

		$sm=new Ser_moni("ts_4xx");
		$sm->update($regs[3]);

		$sm=new Ser_moni("ts_3xx");
		$sm->update($regs[4]);

		$sm=new Ser_moni("ts_2xx");
		$sm->update($regs[5]);
...
//		$q_part2.=", ".$regs[1].", ".$regs[2].", ".$regs[3].", ".$regs[4].", ".$regs[5];
	...
	if (ereg("200: ([0-9]+) 202: ([0-9]+) 2xx: ([0-9]+)", $out_l, $regs)){
		...
		$sm=new Ser_moni("sl_200");
		$sm->update($regs[1]);

		$sm=new Ser_moni("sl_202");
		$sm->update($regs[2]);

		$sm=new Ser_moni("sl_2xx");
		$sm->update($regs[3]);
...
//		$q_part2.=", ".$regs[1].", ".$regs[2].", ".$regs[3];
	...
	if (ereg("300: ([0-9]+) 301: ([0-9]+) 302: ([0-9]+) 3xx: ([0-9]+)", $out_l, $regs)){
		...
		$sm=new Ser_moni("sl_300");
		$sm->update($regs[1]);

		$sm=new Ser_moni("sl_301");
		$sm->update($regs[2]);

		$sm=new Ser_moni("sl_302");
		$sm->update($regs[3]);

		$sm=new Ser_moni("sl_3xx");
		$sm->update($regs[4]);
...
//		$q_part2.=", ".$regs[1].", ".$regs[2].", ".$regs[3].", ".$regs[4];
	...
	if (ereg("400: ([0-9]+) 401: ([0-9]+) 403: ([0-9]+) 404: ([0-9]+) 407: ([0-9]+) 408: ([0-9]+) 483: ([0-9]+) 4xx: ([0-9]+)", $out_l, $regs)){
		...
		$sm=new Ser_moni("sl_400");
		$sm->update($regs[1]);

		$sm=new Ser_moni("sl_401");
		$sm->update($regs[2]);

		$sm=new Ser_moni("sl_403");
		$sm->update($regs[3]);

		$sm=new Ser_moni("sl_404");
		$sm->update($regs[4]);

		$sm=new Ser_moni("sl_407");
		$sm->update($regs[5]);

		$sm=new Ser_moni("sl_408");
		$sm->update($regs[6]);

		$sm=new Ser_moni("sl_483");
		$sm->update($regs[7]);

		$sm=new Ser_moni("sl_4xx");
		$sm->update($regs[8]);
...
//		$q_part2.=", ".$regs[1].", ".$regs[2].", ".$regs[3].", ".$regs[4].", ".$regs[5].", ".$regs[6].", ".$regs[7].", ".$regs[8];
	...
	if (ereg("500: ([0-9]+) 5xx: ([0-9]+)", $out_l, $regs)){
		...
		$sm=new Ser_moni("sl_500");
		$sm->update($regs[1]);

		$sm=new Ser_moni("sl_5xx");
		$sm->update($regs[2]);
...
//	if (!$res) {$errors[]="error in SQL query, line: ".__LINE__; break;}
	...
	//	example: "'aliases' 408 0"
		if (ereg("'([^']*)' ([0-9]+) ([0-9]+)", $out_l, $regs)){
			...
			$sm=new Ser_moni("ul_".$regs[1]."_reg");
			$sm->update($regs[2]);
			
			$sm=new Ser_moni("ul_".$regs[1]."_exp");
			$sm->update($regs[3]);
