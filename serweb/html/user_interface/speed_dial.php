...
do{
	...
	if ($dele_sd){
		$q="delete from ".$config->table_speed_dial." where ".
			"username='".$auth->auth["uname"]."' and domain='".$config->realm."' and username_from_req_uri=".$dele_sd;
		$res=mySQL_query($q);
	...
	if ($edit_sd){
		$q="select username_from_req_uri, new_request_uri from ".$config->table_speed_dial.
			" where domain='".$config->realm."' and username='".$auth->auth["uname"]."' and username_from_req_uri=".$edit_sd;
		$res=mySQL_query($q);
		if (!$res) {$errors[]="error in SQL query, line: ".__LINE__; break;}
	...
	if (isset($okey_x)){								// Is there data to process?
		...
		if ($edit_sd) $q="update ".$config->table_speed_dial." set new_request_uri='$new_uri', username_from_req_uri='$usrnm_from_uri' ".
			"where username_from_req_uri='$edit_sd' and domain='".$config->realm."' and username='".$auth->auth["uname"]."'";
		else $q="insert into ".$config->table_speed_dial." (username, domain, username_from_req_uri, new_request_uri) ".
			"values ('".$auth->auth["uname"]."', '".$config->realm."', '$usrnm_from_uri', '$new_uri')";
...
do{
	if ($db){
		// get speed dials
		if ($edit_sd) $qw=" and username_from_req_uri!='$edit_sd' "; else $qw="";

		$q="select username_from_req_uri, new_request_uri from ".$config->table_speed_dial.
			" where domain='".$config->realm."' and username='".$auth->auth["uname"]."'".$qw." order by username_from_req_uri";
...
<tr><td>
	...
	<tr>
	<td class="titleT">username from request uri</td>
	...
	<td align="left" class="f12">&nbsp;<?echo $row->username_from_req_uri;?></td>
	<td width="2" bgcolor="#C1D773"><img src="<?echo $config->img_src_path;?>title/green_pixel.gif" width="2" height="2"></td>
	<td align="left" class="f12"><?echo $row->new_request_uri;?></td>
	<td width="2" bgcolor="#C1D773"><img src="<?echo $config->img_src_path;?>title/green_pixel.gif" width="2" height="2"></td>
	<td align="center" class="f12"><a href="<?$sess->purl("speed_dial.php?kvrk=".uniqID("")."&edit_sd=".$row->username_from_req_uri);?>">edit</a></td>
	<td width="2" bgcolor="#C1D773"><img src="<?echo $config->img_src_path;?>title/green_pixel.gif" width="2" height="2"></td>
	<td align="center" class="f12"><a href="<?$sess->purl("speed_dial.php?kvrk=".uniqID("")."&dele_sd=".$row->username_from_req_uri);?>">delete</a></td>
	</tr>
	<?}?>
	</table>
</td></tr>
