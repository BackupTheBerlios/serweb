...
//        <script type="text/javascript" src="../../dialog.js"></script>
		...
		function onOK() {
			...
			var f_callback = formObj.callback.value;
			...
			if (window.opener) {
				...
				if (f_callback != "" && window.opener){
					eval("window.opener."+f_callback+"(f_url)");
					top.close();
					return;
				}
		...
		function moveFile() {
			...
			if (!confirm('<?php echo $MY_MESSAGES['renamewarning']; ?>')) return false;
			var postForm2 = fileManager.document.getElementById('form2');
			Dialog("<?php $sess->purl("move.php"); ?>", function(param) {
		...
		function openFile() {
			...
			for (var i=0; i<folderItemsLength; i++) {
				var strId = folderItems[i].getAttribute("id").toString();
				var trId = parseInt(strId.substring(1, strId.length));
				var win = window.open(urlPrefix+myPath+fileManager.folderJSArray[trId][1], 
										"filemanagerPopup" + new Date().getTime(), 
										'toolbar=no,location=no,directories=no,status=yes,menubar=no,scrollbars=no,resizable=yes,width=200,height=200');
			}
			for (var i=0; i<fileItemsLength; i++) {
				var strId = fileItems[i].getAttribute("id").toString();
				var trId = parseInt(strId.substring(1, strId.length));
				var win = window.open(urlPrefix+myPath+fileManager.fileJSArray[trId][1], 
										"filemanagerPopup" + new Date().getTime(), 
										'toolbar=no,location=no,directories=no,status=yes,menubar=no,scrollbars=no,resizable=yes,width=200,height=200');
			}
		}
...
<body onload="Init();">
                ...
                <form action="<?php $sess->purl('files.php?dialogname='.$MY_NAME); ?>" name="form1" method="post" target="fileManager" enctype="multipart/form-data">
                          ...
                          <fieldset>
                                ...
                                <div style="margin:5px;">
                                     ...
                                     ?>
                                                        ...
                                                        <input id="callback" type="hidden" value="<? echo !empty($_REQUEST['callback']) ? $_REQUEST['callback'] : ""; ?>" />
                                </div>
                                ...
                                <!--
                                        ...
                                        -->
                                   <iframe src="<?php $sess->purl('files.php?dialogname='.$MY_NAME); ?>&amp;refresh=1" name="fileManager" id="fileManager" background="Window" marginwidth="0" marginheight="0" valign:"top" scrolling="no" frameborder="0" hspace="0" vspace="0" width="600px" height="250px" style="background-color: Window; margin:0px; padding:0px; border:0px; vertical-align:top;"></iframe>
                                </div>
